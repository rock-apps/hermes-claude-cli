...
**This plugin does not run an HTTP server.** The obvious approach — replicating `claude-bridge` as an embedded HTTP server — was evaluated and rejected in [`docs/04-is-bridge-necessary.md`](./docs/04-is-bridge-necessary.md). Instead it uses Hermes Agent's native mechanism for non-HTTP providers:
...
- Same pattern as Hermes Agent's own bundled reference provider: `plugins/model-providers/copilot-acp/` + `agent/copilot_acp_client.py` (see `docs/03-hermes-provider-model.md`).

If a future session considers "adding an HTTP server back", read `docs/04-is-bridge-necessary.md` and `docs/09-scope-and-migration.md` first — there's exactly one documented scenario where that would make sense (reuse by non-Hermes tools), tracked as an optional, undemanded Fase 6.
...
- `claude --help` output — flags documented in `docs/06-claude-cli-reference.md`. Revalidate if the installed CLI version changes.
