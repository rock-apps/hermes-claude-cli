...
It replaces `CLAUDE_CLI_MCP_CONFIG`/`CLAUDE_CLI_ALLOWED_TOOLS` in place (backing up the previous `.env` as `.bak-<UTC timestamp>` first) and leaves every other line untouched. Onboarding a new profile means: install the plugin for it (`hermes -p <name> plugins install rock-apps/hermes-claude-cli/plugin/claude_cli --force --enable`), re-run both commands above with the new name added to `--profiles`, and restart the gateway *and* `hermes-webui` if it's running — still three manual steps, not zero, because of the `create_client()` limitation above, but no more manual JSON editing.
