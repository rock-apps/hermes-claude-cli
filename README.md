...
- **Hermes' native tools (cron/schedule creation, `RemoteTrigger`, kanban, etc.) don't work through this provider.** The `claude` CLI doesn't accept externally-defined tool schemas — it only knows its own built-in tools. Hermes exposes its native tools to the model via the OpenAI-style `tools:` request field, which never reaches the `claude` subprocess, so the model has no way to know those tools exist. Confirmed on a live deployment: asking for a recurring scheduled check correctly gets "I don't have that tool" from `claude-cli`, while the identical request against an OpenAI-compatible provider (e.g. `deepseek` via OpenRouter) works normally.
  ...
  - **Guidance**: use a different provider for profiles/workflows that depend on Hermes' native scheduling/automation tools; use `claude-cli` for reasoning, coding, and chat. See [`docs/06-claude-cli-reference.md`](./docs/06-claude-cli-reference.md#what-the-cli-still-cant-do-true-regardless-of-architecture) for details.
  - **This is not fixable by switching to the Claude Agent SDK.** The Agent SDK supports custom tools but requires `ANTHROPIC_API_KEY` billing — it cannot reuse a Claude Max subscription's OAuth session, per Anthropic's own docs. Evaluated and rejected; see [`docs/09-scope-and-migration.md`](./docs/09-scope-and-migration.md).
