...
|---|---|---|
| Network | Unauthenticated HTTP server on `:9180`, bound to `0.0.0.0` (not `127.0.0.1`, despite the README claiming otherwise — see [01](./01-analise-claude-bridge.md)). Any process reaching the port spends the user's subscription. | **No open port.** The only channel is stdio to a child subprocess of Hermes' own process. No network surface to defend. |
...
| Uncontrolled spend | No per-call spend cap. | `--max-budget-usd` is available natively (see [06](./06-referencia-cli-claude.md)) and exposed as `CLAUDE_CLI_MAX_BUDGET_USD`, though unset by default (no cap). |
