...
Hermes Agent's native `auth_type="external_process"` + `create_client()` extension point (used by the bundled `copilot-acp` provider) makes an HTTP server unnecessary — see [ADR 04](./04-is-bridge-necessary.md). Auditing the real Go source also surfaced problems worth not repeating:
...
- **Streaming was fake.** No `--output-format stream-json`; "streaming" was one full response split into 3 SSE events after the CLI already finished. The `claude` CLI does support real incremental output (`stream-json --include-partial-messages`) — see [06](./06-claude-cli-reference.md).
...
- A single "backup" commit also introduced two undocumented, unrelated binaries (`model-router`, `zai-proxy` — routing the *Claude Code CLI itself* to third-party models like DeepSeek/GLM) plus several binary/`.pid` artifacts that shouldn't have been committed. Out of scope here — see [09-scope-and-migration.md](./09-scope-and-migration.md).
