...
| `--resume <id>` | Resumes a prior session by the `session_id` a previous call returned, so only the new message(s) need to be sent — see Fase 4 in [08-roadmap.md](./08-roadmap.md). |
| `--add-dir <dirs...>` | Grants read access to directories outside the cwd. Variadic — see the `--` note below. |
| `--dangerously-skip-permissions` | Bypasses the permission system entirely. Available in `process.PermissionConfig.bypass` but not exposed via any environment variable — opt-in only by editing `client.py` directly (see [06-security.md](./06-security.md)). |
...
| `--restricted` | Drops Bash/PowerShell/REPL/WebFetch. Defaults to on (`CLAUDE_CLI_RESTRICTED=true`) — see [06-security.md](./06-security.md). |
| `--max-budget-usd <amount>` | Per-call spend cap, exposed as `CLAUDE_CLI_MAX_BUDGET_USD` (unset by default). |
| `--` (separator) | Always inserted right before the prompt. `--add-dir` is variadic, so without this a prompt not starting with `-` gets silently swallowed as one more directory instead of reaching `claude` — a real bug found and fixed during Fase 3, see [06-security.md](./06-security.md). |
| `--output-format stream-json` | Used for `stream=True` calls (`process.run_streaming()`) — real incremental output, one JSON event per line. See Fase 2 in [08-roadmap.md](./08-roadmap.md) for why this was initially skipped, then built after all. |
...
- **No passthrough for externally-defined `tools:`.** The `claude` CLI doesn't accept tool schemas from a caller — it only knows its own built-in tools (Read, Grep, Bash, Edit, etc.). This limitation, documented by both original third-party projects, persists here too — it's a CLI limitation, not a transport one.
  ...
  - **Fixed for cron/kanban specifically**: see [`mcp-bridge/`](../mcp-bridge/) — a small MCP server wrapping the `hermes cron`/`hermes kanban` CLI, registered via `claude mcp add`, confirmed working against a real Max subscription session. See the "Resolved" entry in [07-scope-and-migration.md](./07-scope-and-migration.md) for how and why this works where per-request `tools:` doesn't.
  - **Practical guidance for anything else** (e.g. `delegate_task`, not covered by the bridge above): use a real OpenAI-compatible provider (Nous, OpenRouter, etc.) instead of `claude-cli` for that specific profile/workflow; reserve `claude-cli` for reasoning/coding/chat and MCP-backed lookups, where it performs well.
  - **Not fixable by switching to the Claude Agent SDK.** The Agent SDK does support custom tool schemas (MCP servers, `allowedTools`) — but it requires `ANTHROPIC_API_KEY` (or a cloud provider auth) and does not support reusing a Claude Max subscription's local OAuth session; Anthropic's own docs state third-party SDK apps may not offer claude.ai login. Adopting it would fix the tool passthrough at the cost of the entire reason this project exists (Max-plan billing instead of per-token API credits). See [07-scope-and-migration.md](./07-scope-and-migration.md) for the full evaluation.
